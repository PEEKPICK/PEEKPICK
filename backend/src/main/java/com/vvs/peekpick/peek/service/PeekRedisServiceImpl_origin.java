...
//public class PeekRedisServiceImpl_origin implements PeekRedisService {
//
...
////        Duration ttl = Duration.between(peekDto.getWriteTime(), peekDto.getFinishTime());
...
//                .build();
//
//        hashOps.put(PEEK_REDIS, peekId.toString(), updatedPeekDto);
//
//        // PeekDto의 finishTime으로 TTL 재설정
//        Duration ttl = Duration.between(LocalDateTime.now(), updatedPeekDto.getFinishTime());
//        peekTemplate.expire(peekId.toString(), ttl);
//        locationTemplate.expire(peekId.toString(), ttl);
//
