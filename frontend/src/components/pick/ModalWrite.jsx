...
const ModalWrite = ({ emojiCall }) => {
  ...
  const postWrite = () => {
    ...
    f.append("longitude", myPos.point.x);
    f.append("latitude", myPos.point.y);
  ...
  return (
    <>
      <Modal isOpen={isModalState} onRequestClose={() => handleCloseModal()} className={classes.modalMain}>
        ...
        <div className={classes.name}>{imgData.name}</div>
        <div className={classes.imgthrow}>
          <button onClick={onCickImageUpload} className={classes.btn}>
            이미지업로드
          </button>
          <input ref={imageInput} type="file" accept="image/*" onChange={imgAccept} className={classes.imginput} />
        </div>
        <div className={classes.imgthrow}>
          <button className={classes.button} onClick={postWrite}>
            입력 완료
          </button>
        </div>
      </Modal>
